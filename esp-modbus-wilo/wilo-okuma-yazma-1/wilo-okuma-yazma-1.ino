...
int cbWrite(Modbus::ResultCode event, uint16_t transactionId, void* data) {
  ...
  Serial.print("cbWrite event = ");Serial.println(event);
  error_code=event;
  return true;
};
...
int register_pump_heartbeat=500;//2 byte read//https://cms.media.wilo.com/dcidocpfinder/wilo181064/1552306/wilo181064.html
int register_pump_error=940;//1 byte read
uint16_t Mread0[1];
...
*/
   if (!mb.slave()) {
    ...
    uint16_t regs22[2] = {9,5};//4002 ye role on komutu
    mb.writeHreg(3,40, (uint16_t*)&regs22, 2, cbWrite);
